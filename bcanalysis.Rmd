...
A note on the programming. I really love R's tidyverse and plotting capabilities, but I prefer Python for scripting (and almost everything else). The problem was finding a way that I could unify the output of both without running two scripts in parallel that parsed the output of one another/some other hacky and horrible way. I found the `reticulate` library that allows interoperability and the sharing of states between R and Python code, which in turn has allowed me to combine my R and Python scripts into one .Rmd file. 

I'm not sure whether it's horrible or not:
